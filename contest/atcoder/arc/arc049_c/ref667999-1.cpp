...
int main(void)
{
	...
	int answer = 0;
	for (int i = 1; i <= 1 << B; ++i) {	// B 個のタイプ 2 制約それぞれについて 2 種類の場合を総当り法で調べる。場合の数は 2**B 通り、すなわち 1 << B 通りである。B が最大値 10 の時 2**B は 1024 である。
		for (int j = 1; j <= N; ++j)
		...
		for (int j = 1; j <= B; ++j) {	// タイプ 2 制約それぞれについて。
			if ((i - 1) >> (j - 1) & 1)	// B ビットの i の中で、今、注目しているタイプ 2 制約 j に当たる部分は立っているか？
